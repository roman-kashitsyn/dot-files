...
main = yi $ myConfig
  ...
  , configUI = defaultUIConfig
     ...
     , configTheme = defaultLightTheme `override` \super _ -> super
       { keywordStyle       = withBd True `mappend` withFg blue
       , importStyle        = withBd True `mappend` withFg cyan
       , modelineAttributes = emptyAttributes { foreground = black, background = green }
       , tabBarAttributes   = emptyAttributes { foreground = black, background = green }
       }
